...
dvar int estActif[codeTaches] in 0..1;
dvar int cumulPuissance[0..sommeDuree] in 0..puissanceMax;
...
subject to {	
	...
	(sum(c in codeTaches) pulse(t[c] , ct[c].puissance)) <= puissanceMax;
	
	forall(instantT in 0..sommeDuree) {
		forall (c in codeTaches) {
			estActif[c] == ((instantT >= startOf(t[c])) && (instantT <= endOf(t[c])));
		}
		
		cumulPuissance[instantT] == (sum(c in codeTaches) (estActif[c]*ct[c].puissance));		
	} 
...
execute {
	...
	for (instantT in 0..finChantier) {
		writeln("Consommation instant ", instantT, " = ", cumulPuissance[instantT]);
	}	
}
